...
#### SYSTEM BUILD
SYSTEM_CPPS := $(wildcard src/$(NAME)/*.cpp)
SYSTEM_OBJ := $(BUILD)/$(NAME).o
...
$(BUILD)/$(NAME)/$(NAME).o: src/$(NAME)/$(NAME).cpp
	@mkdir -p $(dir $@)
	$(CXX) -o $@ -c $^ -Isrc/$(NAME)/include $(CXXFLAGS)

$(SYSTEM_OBJ): $(COMPONENTS_OBJS) $(BUILD)/$(NAME)/$(NAME).o
	@mkdir -p $(dir $@)
	$(LD) -o $@ -r $^
...
$(BIN)/$(NAME).ut: $(UT_COMPONENTS_OBJS) $(SYSTEM_OBJ) $(BUILD)/utmain.o
	@mkdir -p $(dir $@)
	$(CXX) -o $@ $^ $(CXXFLAGS)
